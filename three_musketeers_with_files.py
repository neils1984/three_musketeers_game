...
def create_save_file(f_name, users_side):
    """Creates a text file using a given filename that contains 2 lines
    one line contains all the pieces in each location of the board and the
    other contains the users side"""
    f_name = f_name.replace('.txt','')
    f_out = open(f_name + '.txt', 'w') # makes sure the filename contains '.txt' but only once
    board_list = [at(x) for x in all_locations()]
    for x in board_list:
        f_out.write(x + ' ')
    f_out.write('\n')
    f_out.write(users_side)
    f_out.close()
    print('Game saved.')
          
def load_saved_file():
    """Returns a tuple of a board and the users side from a saved game text file"""
    gameloaded = False
    while gameloaded == False:
        try:
            saved_file = open(input('Please enter the saved game file to load: ').replace('.txt','') + '.txt') #makes sure '.txt' is in the filename but only once
            saved_board_list = saved_file.readline().strip().split()
            saved_board = [saved_board_list[0:5],\
                           saved_board_list[5:10],\
                           saved_board_list[10:15],\
                           saved_board_list[15:20],\
                           saved_board_list[20:25]]
            users_side = saved_file.readline().strip()
            gameloaded = True
        except IOError:
            print('File not found.')
    saved_file.close()       
    return (saved_board, users_side)
    
def ask_load_game():
    """Asks whether a player wants to load a previously saved game"""
    loadgame = input('Do you want to load a saved game? Y/N: ')
    if loadgame.upper() == 'Y':
        return True
    elif loadgame.upper() == 'N':
        return False
    return ask_load_game()

def ask_quit_game():
    """Asks whether a player wants to quit the game or continue playing"""
    cont = input('Do you want to (C)ontinue playing or (Q)uit the game? ')
    if cont.upper() == 'C':
        return False
    elif cont.upper() == 'Q':
        print('Thank you for playing!')
        return True
    return ask_quit_game()

def input_with_save_option(users_side):
    move = input("Your move? ").upper().replace(' ', '')
    quit_game=False
    if move.upper() == 'S': # user input to save game
        create_save_file(input('Save game as: '), users_side)
        quit_game = ask_quit_game() # asks if user wants to continue playing or quit the game
        if not quit_game:
            return input_with_save_option(users_side)
...
def get_users_move(users_side):
    ...
    inp = input_with_save_option(users_side)
    move = inp[0]
    quit_game = inp[1]         
    if not quit_game:
        if (len(move) >= 3
            ...
            print("Illegal move--'" + move + "'")
            return get_users_move(users_side)
    return (None,None) #produces a dummy return tuple that can be checked for None type later to confirm the player wants to quit.
...
def move_musketeer(users_side):
    ...
    if users_side == 'M':
        ...
        if location == None: # if the player has chosen to quit
            return None
        if at(location) == 'M':
            if is_legal_move(location, direction):
...
def start():
    ...
    if ask_load_game(): #asks whether user wants to load a game
        game_to_load = load_saved_file()
        board = set_board(game_to_load[0])
        users_side = game_to_load[1]
        print('\nGame loaded...')
        if users_side == 'M':
            player_str = 'Musketeers'
        else:
            player_str = 'enemy'
        print('\nPlayer is playing as', player_str)
    ...
    print_instructions()
    print_board()
    if game_was_loaded and users_side == 'R': # stops the computer taking two turns if the player is playing as enemy when game is reloaded
        if has_some_legal_move_somewhere('R'):
            board = move_enemy(users_side)
    if not board == None: #continue if player has NOT chosen to save and quit immediately after loading the game
        print_board()
        while True:
            if has_some_legal_move_somewhere('M'):
                board = move_musketeer(users_side)
                if board == None: # if the player has chosen to quit
                    break
                print_board()
                if is_enemy_win():
